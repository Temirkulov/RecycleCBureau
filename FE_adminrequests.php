...
if (isset($_GET['search'])) {
    ...
    $sql = "SELECT * FROM request WHERE centre_id LIKE '%$search%' OR username LIKE '%$search%' OR notes LIKE '%$search%' OR street LIKE '%$search%' OR city LIKE '%$search%' OR doc LIKE '%$search%';";
} else {
...
<h2 class="logo">Logo</h2>
		...
		<nav class="navigation">
			...
			<div class="search">
			    <form action="BE_adminrequests.php" method="get">
...
<section class="about">
    ...
    <p>
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            echo '<thead><tr style="border-bottom: 1px solid white;color: white;"><th>ID</th><th>User ID</th><th>Centre ID</th><th>Username</th><th>notes</th><th>street</th><th>city</th><th>doc</th><th>Action</th></tr></thead>';
            echo '<tbody>';
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '<td style="border-right: 1px solid white; height: 50px;color: white;">' . $row["user_id"] . '</td>';
                echo '<td style="border-right: 1px solid white; height: 50px;color: white;">' . $row["centre_id"] . '</td>';
                ...
                echo '<td style="border-right: 1px solid white; height: 50px;color: white;">' . $row["doc"] . '</td>';
                echo '<td style="border-right: 1px solid white; height: 50px;color: white;">' . '<a href="BE_request.delete.php?id=' . $row["id"] . '">Delete</a>' . '</td>';
                echo '</tr>';
            }
